...
class BLEDevice:
    ...
    def connect(self, addr):
        ...
        self.gatt = pexpect.spawn("gatttool -t random -b " + addr + " -I")
        self.gatt.expect('\[LE\]>', timeout=300)
        self.gatt.sendline('connect')
        self.gatt.expect('Connection successful.*\[LE\]>', timeout=300)
    ...
    def writereq(self, handle, value):
        req = "char-write-req 0x%04x %s" % (handle, value) +" --listen"
        print(req)
        #cmd = "char-write-cmd 0x%02x %s" % (handle, value.encode('hex'))
    ...
    def notify(self):
        while True:
            try:
                num = self.gatt.expect('Notification handle = .*? \r', timeout=0.01)
            except pexpect.TIMEOUT:
                break
            if num == 0:
